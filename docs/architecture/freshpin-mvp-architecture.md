...
2. 服务端验证会话、名称和图片二进制格式，生成 `userId/<uuid>.<ext>` 私有对象路径。图片先经过 magic bytes/尺寸预筛，再由仅服务端的解码器实际解码；仅通过头部检查的伪造、损坏或动画图片不可入库。
...
- 所有业务页面的受保护布局渲染统一 `AppHeader`：品牌链接的可访问名称为“返回首页”，目标固定为 `/`。流程页不依赖浏览器后退作为唯一退出路径。
- 登录态的产品承诺是“首次登录后 30 天内免重复登录”。Supabase 接入时保持短 JWT（默认 1 小时）和刷新令牌轮换，不将 JWT 延长至 30 天；在 Auth Sessions 中将 session time-box 设为 `720h`，到期后要求重新邮箱验证。time-box 是 Supabase Pro 及以上能力；若部署计划不支持，应用层必须以服务端可验证的首次登录时间实现等价强制过期，不能仅依赖浏览器 cookie 的过期时间。
...
导航与会话验收：每个非首页业务页均可通过“返回首页”链接回到 `/`；用户在第 29 天仍可访问已登录内容，在第 30 天会话上限后必须重新完成邮箱验证。
